...
vector<Ellipse> get_ellipse_array(std::vector<Obstacle> &seg_obs, const grid_map::GridMap &localmap_obs, const LocalMapParam &MapParam)
{
	std::vector<Ellipse> result;
	float localmap_x_size = MapParam.localmap_x_size;
	float localmap_y_size = MapParam.localmap_y_size;
	float resolution = MapParam.resolution;
	float _inv_resolution = 1.0 / resolution;
